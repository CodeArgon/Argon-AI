...
const LeadsList = () => {
  ...
  useEffect(() => {
    applyFilters();
  }, [dateRange, status, sourceFilter, mediumFilter]);
  // Filter function
  const applyFilters = () => {
    ...
    console.log("mediumFilter: ", mediumFilter);
    ...
    if (sourceFilter && sourceFilter.value != null) {
      updatedData = updatedData.filter(
        (lead) => lead.source === sourceFilter.value
      );
    }
    if (mediumFilter && mediumFilter.value != null) {
      updatedData = updatedData.filter(
        (lead) => lead.medium === mediumFilter.value
      );
    }
    // else {
    //   updatedData = [...data];
  ...
  const handleSourceChange = (selectedOption) => {
    setSourceFilter(selectedOption);
  };
  const handleMediumChange = (selectedOption) => {
    setMediumFilter(selectedOption);
  };
  const customStyles = {
